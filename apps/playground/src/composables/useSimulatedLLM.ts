...
export type Persona = 'helpful-assistant' | 'code-tutor' | 'story-narrator'

export interface UseSimulatedLLMReturn {
  /** Whether a stream is currently being generated. */
  readonly isStreaming: ReturnType<typeof readonly<ReturnType<typeof shallowRef<boolean>>>>
  /**
   * Start streaming a random scripted response for the current persona.
   * Returns an AsyncGenerator<string> that yields small text chunks.
   * Pass it directly to `useStreamingTTS().pipeStream()`.
   *
   * Calling start() while streaming cancels the previous stream first.
   */
  start(): AsyncGenerator<string>
  /**
   * Abort the current stream immediately. (E-P2.3b: safe to call before start)
...
const SCRIPTS: Record<Persona, string[]> = {
  'helpful-assistant': [
 ...
 * ```
 */
export function useSimulatedLLM(
  persona: Persona,
  msPerToken = 30,
): UseSimulatedLLMReturn {
  ...
  async function* makeStream(text: string, signal: AbortSignal): AsyncGenerator<string> {
  ...
  function start(): AsyncGenerator<string> {
